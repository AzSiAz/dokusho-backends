...
func (m *mangadex) FetchSerieDetail(context context.Context, serieID source_types.SourceSerieID) (source_types.SourceSerie, error) {
	...
	// TODO(stef): Add some point there will be a need add a check to see if the chapter is already in the list, since the total could change if another chapter is added
	for {
		q := volumesURL.Query()
		q.Set("offset", strconv.Itoa(offset))
		volumesURL.RawQuery = q.Encode()
