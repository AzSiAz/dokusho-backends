...
fn serialize_skip_none<S>(
    map: &HashMap<SourceLanguage, Option<Vec<String>>>,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
    let filtered: HashMap<&SourceLanguage, &Vec<String>> = map
        .iter()
...
fn deserialize_with_none<'de, D>(
    deserializer: D,
) -> Result<HashMap<SourceLanguage, Option<Vec<String>>>, D::Error>
where
    D: Deserializer<'de>,
{
...
pub struct MultiLanguageString {
    ...
    value: HashMap<SourceLanguage, Option<Vec<String>>>,
}
        ...
        self.value
            ...
            .and_modify(|entry| {
                if let Some(vec) = entry {
                    ...
                    *entry = Some(vec![text.clone()]);
                }
            })
            .or_insert(Some(vec![text]));
        self
    }
...
mod tests {
    ...
    fn test_multilanguagestring_serialize_skip_none() {
        ...
        map.insert(SourceLanguage::En, Some(vec!["Hello".to_string()]));
        map.insert(SourceLanguage::Jp, Some(vec!["こんにちは".to_string()]));
    ...
    fn test_multilanguagestring_deserialize() {
        ...
        assert_eq!(
            ...
            Some(&Some(vec!["こんにちは".to_string()]))
        );
        assert_eq!(multi_lang.value.get(&SourceLanguage::Fr), None);
    }

    #[test]
    fn test_multilanguagestring_insert() {
        let multi_lang = MultiLanguageString::new()
            .insert(SourceLanguage::En, "Hello".to_string())
            .insert(SourceLanguage::En, "World".to_string())
            .insert(SourceLanguage::Jp, "こんにちは".to_string());

        assert_eq!(
            multi_lang.value.get(&SourceLanguage::En),
            Some(&Some(vec!["Hello".to_string(), "World".to_string()]))
        );
        assert_eq!(
            multi_lang.value.get(&SourceLanguage::Jp),
            Some(&Some(vec!["こんにちは".to_string()]))
