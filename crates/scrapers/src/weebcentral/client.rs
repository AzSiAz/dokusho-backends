...
use dokusho_core::{
    ...
    SourceApiInformation, SourceError, SourceInformation, SourceLanguage, SourceSerieStatus,
...
impl WeebCentral {
    pub fn new(flaresolver_url: Option<String>) -> Result<Self, SourceError> {
        let mut client = CloudflareAwareHttpClient::new()
            .map_err(|e| SourceError::Other(e.into()))?;
        
        if let Some(url) = flaresolver_url {
            client = client.with_flaresolver(url)
                .map_err(|e| SourceError::Other(e.into()))?;
        }
    ...
    #[cfg(test)]
    pub fn new_with_url(flaresolver_url: Option<String>, base_url: String) -> Result<Self, SourceError> {
    ...
    #[cfg(test)]
    pub fn new_for_testing(base_url: String) -> Result<Self, SourceError> {
        // For testing, create without FlareSolver
        let client = CloudflareAwareHttpClient::new()
            .map_err(|e| SourceError::Other(e.into()))?;
        ...
        let source = Source {
            source_information: SourceInformation {
                ...
                name: "WeebCentral".to_string(),
                url: base_url.clone(),
                icon: format!("{}/favicon.ico", base_url),
                languages: vec![SourceLanguage::En],
                updated_at: chrono::Utc::now(),
                version: "1.0.0".to_string(),
                nsfw: false,
    ...
    async fn fetch_with_search_data_api(
        ...
        url.push_str("&official=Any");
        url.push_str("&anime=Any");
        url.push_str("&adult=Any");
        url.push_str("&display_mode=");
        url.push_str(&urlencoding::encode("Full Display"));

        // Add query
        if !filters.query.is_empty() {
            url.push_str(&format!("&text={}", urlencoding::encode(&filters.query)));
            // When there's a query, use "Best Match" as default sort
            url.push_str("&sort=Best Match");
        } else {
            // Add sort for browsing (no query)
            if let Ok(wc_sort) = WeebCentralSort::try_from(filters.sort) {
                url.push_str(&format!("&sort={}", wc_sort.as_str()));
            } else {
                url.push_str("&sort=Latest Updates"); // Default to Latest Updates
            }
        }
    ...
    async fn fetch_serie_with_chapters(
        ...
        let (serie_html, chapters_html) = tokio::try_join!(
            async { self.client.get_html(&serie_url).await
                .map_err(|e| SourceError::Other(anyhow::anyhow!("HTTP error: {}", e))) },
            async { self.client.get_html(&chapters_url).await
                .map_err(|e| SourceError::Other(anyhow::anyhow!("HTTP error: {}", e))) }
        )?;
...
impl SourceApi for WeebCentral {
    ...
    async fn fetch_chapter_data(
        ...
        let url = format!(
            "{}/chapters/{}/images?reading_style=long_strip",
            self.base_url,
            chapter_id.as_str()
        );
